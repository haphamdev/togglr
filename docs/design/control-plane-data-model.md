...
| `environments` | `id`, `organization_id`, `project_id`, `key`, `name`, `ruleset_version bigint not null default 0`, `archived_at (nullable)`, `created_at` | **`ruleset_version`** = per-env monotonic counter. **`archived_at`** = reversible archive; dashboard lifecycle — SDK hot path unaffected (archived env's keys keep serving until revoked). |
