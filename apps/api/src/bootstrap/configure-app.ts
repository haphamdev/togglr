 ...
 * doc's control-plane base, togglr-api.md:30), excluding `/healthz` (an
 * infra-level probe) and `/sdk/v1/ruleset` (the SDK delivery plane, served at its
 * own `sdk/v1` base) which stay unprefixed.
 *
...
export function configureApp(app: INestApplication): void {
  app.setGlobalPrefix("api/v1", {
    exclude: [
      ...
      { path: "sdk/v1/ruleset", method: RequestMethod.ALL },
    ],
  });
}
